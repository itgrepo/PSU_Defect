    ...
    <v-dialog persistent v-model="dialogEdit" max-width="800px" >
      <v-card>
        <v-toolbar class="light-blue darken-4 white--text color-card-title" dense>
          <v-toolbar-title>{{ $t('editsubcategory') }}</v-toolbar-title>
            <v-spacer></v-spacer>
            ...
            <v-col cols="12">
              <v-flex sm12 md12 xs12>
                    ...
                    <v-text-field :label="$t('selectpng')" @click='$refs.filePng.click()' v-model='nameFilePNG' prepend-icon="attach_file" :rules="Rules"></v-text-field>
                        ...
                        <v-col cols="12">
                          <v-text-field v-model="category_name"  :rules="Rules" outlined solo></v-text-field>
                        </v-col>
                        ...
                        <v-col cols="12">
                          <v-textarea v-model="sub_category_name"  auto-grow outlined solo rows="3" row-height="25" shaped></v-textarea>
                        </v-col>
  ...
  export default {
    ...
    methods: {
      ...
      filesPNG() {
        ...
        formDataICON.append("sub_category_id", this.sub_category_id);
        formDataICON.append("sub_category_name", this.sub_category_name);

        this.axios.post(process.env.VUE_APP_API_BACKEND + "/editSubcategory",
          formDataICON, {
